...
#: 角色三视图整图内的格位顺序（**从左到右**），即 sheet_split 切分产出的视角键。
#: ⚠️ 必须与 comfyui_client._ensure_fullbody_prompt 的版式描述**逐字同序**：
#:    「正面、左侧面、背面三张全身视图从左到右横排」
#: 改这里必须同步改那边的提示词串，否则切出来的 front/left/back 会与实际格位错位。
CHARACTER_SHEET_LAYOUT_ZH = "正面、左侧面、背面"
CHARACTER_SHEET_VIEWS = ("front", "left", "back")

#: 角色/物品/场景资产目录里「派生视角图」的固定名白名单。
#: sheet_split.prune_stale_views 只在这个白名单内清理陈旧文件（不递归、不通配），
#: 用于清掉旧实现遗留的 right.png（右侧半侧面，新实现不再产出）。
ASSET_VIEW_STEMS = ("front", "left", "right", "back")
