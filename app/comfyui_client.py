...
def _env_flag(name: str, default: bool = False) -> bool:
    """环境变量开关（**按字符串语义解析**，空串视为未设置）。

    ⚠️ 别用裸 ``bool(os.environ.get(...))`` —— ``"0"`` 与 ``""`` 都是「非空字符串」，
    裸 bool 会把「明确关闭」判成开启（本项目在音频质检开关上踩过这类坑）。
    """
...
# --------------------------------------------------------------------------- #
# 去水印强化总开关（2026-09-24：**默认关闭**）
# --------------------------------------------------------------------------- #
# 背景：`_harden_no_watermark` / `harden_video_no_text` 会在提交前给提示词追加
# 「无水印/无文字」声明与负向排除词。它曾把**泛称的「文字」**也压进负向词，把
# 「内容本身就该有文字」的资产压成空白 —— 实测「逆天系统」的两件道具：
#   卷轴 `金色半透明卷轴，古篆文字…`      → 出图无文字 → 质检判「缺少古篆文字」→ 无限重试
#   面板 `蓝色半透明文字弹窗，内部任务数值…` → 质检甚至把「无畸变与**文字**」当**合格理由**
# （质检口径就是这条提示词本身 `qc_desc = f"…资产设定：{prompt_zh[:400]}"`，被那句声明带偏）。
#
# 现状：当前图片模型（Qwen-Image 2.1）实测不自行生成水印，这层「从提示词侧双向压制」
# 已无必要，反而污染内容文字。故**默认关闭**。
# 需要时打开：设环境变量 ``MJSCXT_NO_WATERMARK_HARDEN=1``（改后需重启服务）。
WATERMARK_HARDEN_ENABLED = _env_flag("MJSCXT_NO_WATERMARK_HARDEN", False)
...
# API prompt 做强化）：
#   1) 正向提示词统一追加「无水印/无叠加文字」声明；
#   2) 负向提示词统一追加水印类排除词。
# 说明：项目内 4 个图片工作流（角色/物品/场景/分镜生成.json）经核查**不含任何水印/文字叠加
# 节点**，水印只可能由模型自行"画"出来，因此从提示词侧双向压制是彻底去水印的必要手段。
#
# ⚠️⚠️ **绝不能把泛称的「文字」放进负向词表**（2026-09-24 修正，这条实打实踩过）：
# **水印 ≠ 内容文字**。物品/场景/分镜的提示词本身就可能**要求**文字 —— 实测「逆天系统」
# 的两件道具：`金色半透明卷轴，古篆文字…`、`蓝色半透明文字弹窗，内部任务数值…`。
# 一旦把泛称 `文字` 压进负向词、正向又声明「不含任何…文字」，同一段提示词就**自相矛盾**，
# 模型把文字压掉；而**质检口径就是这条提示词本身**（`qc_desc = f"…资产设定：{prompt_zh[:400]}"`），
# 于是质检必然判「缺少古篆文字」→ 无限重试。更荒谬的是另一件道具的质检判词把
# 「无畸变与**文字**」当成**合格理由**（被那句声明带偏）。
# 结论：只压制**叠加物**（水印 / logo / 签名 / 日期戳 / 字幕条 / 标题叠加 / 生成标识），
# 并在正向声明里显式放行「内容本身需要的文字」。
NO_WATERMARK_POSITIVE_SUFFIX = ("。画面干净：不含任何水印、logo、签名、日期戳、版权标识、"
                                "字幕条与叠加标题文字，尤其不得出现「AI生成」等任何生成标识"
                                "文字（含右下角）；但画面内容本身需要的文字（器物铭文、"
                                "书页文字、面板数值等）应正常清晰呈现")
NO_WATERMARK_NEGATIVE_EXTRA = ("水印, watermark, logo, 标志, 签名, 签字, 日期戳, 时间戳, "
                               "字幕, 标题文字, 版权标识, 边框水印, 半透明水印, "
                               "AI生成, AI生成字样, 生成标识, 机器生成, generated by AI")

#: 幂等标记（**版本无关**）：判断槽位是否已加固过。
#: ⚠️ 不能直接拿常量本身比对 —— 常量一旦修订，历史加固过的提示词会被判成"未加固"而
#: **再追加一遍**，新旧两份声明并存；更糟的是旧负向词里还留着泛称「文字」，等于白改。
_WATERMARK_DECL_MARK = "画面干净：不含任何水印"
_WATERMARK_NEG_MARK = "水印, watermark, logo"

# H3 视频链路：画面内**字幕/叠加文字**压制（H3 提示词节点为纯文本节点 Text Multiline）
# ⚠️ 同样不能写「不得出现任何文字」（理由见上面图片链路那段）：逆天系统的面板数值、
# 场景里的牌匾铭文都是视频画面内容的一部分，压成"无文字"会让成片与剧本不符。
VIDEO_NO_TEXT_SUFFIX = ("。画面中不得出现字幕条、台词文本叠加、标题条、水印、logo 或生成标识，"
                        "尤其不得在画面底部、顶部或右下角绘制台词字幕与生成标识；"
                        "画面内容本身需要的文字（器物铭文、面板数值等）应正常呈现")
VIDEO_PROMPT_NODE_TYPES = ("Text Multiline", "Text", "CLIPTextEncode", "PrimitiveStringMultiline")

#: 视频链路无字幕声明的幂等标记（版本无关，理由同上）
_VIDEO_NO_TEXT_MARK = "不得在画面底部、顶部或右下角绘制台词字幕"
...
class ComfyUIClient:
    ...
    def _harden_no_watermark(api_prompt: dict) -> dict:
        ...
        """
        if not WATERMARK_HARDEN_ENABLED:
            return {}       # 见模块顶部「去水印强化总开关」：2026-09-24 起默认关闭
        changed = {}
        for nid, field, value, polarity in _iter_prompt_slots(api_prompt):
            # 幂等：已加固过的槽位直接跳过（否则正向后缀里的"水印"二字会被误判为负向）。
            # ⚠️ 用**版本无关**的标记判定，不能拿常量本身比对 —— 常量修订后旧常量消失，
            # 历史加固过的提示词会被判成"未加固"而再追加一遍（新旧两份声明并存）。
            if _WATERMARK_DECL_MARK in value or _WATERMARK_NEG_MARK in value:
                continue
            if polarity == "neg":
        ...
        for nid in targets:
            ...
            for field in PROMPT_TEXT_FIELDS:
                ...
                if not isinstance(value, str) or not value.strip():
                    continue
                if _VIDEO_NO_TEXT_MARK in value:
                    continue
