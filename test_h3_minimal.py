...
def main():
    ...
    # 获取可用的模型
    diffusion_models = []
    r2 = urllib.request.urlopen(f'{COMFYUI_URL}/models/diffusion', timeout=5)
    diffusion_models = json.loads(r2.read())
    
    # 找到minimax-h3模型
    h3_model = None
    for m in diffusion_models:
        if 'minimax-h3' in m.lower() or 'h3' in m.lower():
            h3_model = m
            break
    
    print(f'使用模型: {h3_model}')
    
    # 获取VAE
    vae_models = []
    r3 = urllib.request.urlopen(f'{COMFYUI_URL}/models/vae', timeout=5)
    vae_models = json.loads(r3.read())
    h3_vae = [v for v in vae_models if 'minimax' in v.lower() or 'h3' in v.lower()]
    print(f'VAE: {h3_vae[:3]}')
    
    # 获取CLIP
    clip_models = []
    r4 = urllib.request.urlopen(f'{COMFYUI_URL}/models/clip', timeout=5)
    clip_models = json.loads(r4.read())
    print(f'CLIP模型: {clip_models[:5]}')
    ...
    # 5. MiniMaxH3ReferenceToVideo - 生成视频
    nodes['5'] = {
    ...
    nodes['6'] = {
        ...
        'inputs': {
            'vae_name': h3_vae[0] if h3_vae else vae_models[0]
        }
    }
    ...
    # 9. SaveVideo - 保存视频（如果CreateVideo不支持）
    nodes['9'] = {
        'class_type': 'SaveVideo',
        'inputs': {
            'filename_prefix': TEST_OUTPUT,
            'ffmpeg_path': r'D:\ComfyUI_portable_TE_v260619\ComfyUI\ffprobe\ffmpeg.exe',
            'video': ['8', 0]
        }
    }
    
    print(f'\n工作流构建完成: {len(nodes)} 个节点')
    ...
    for i in range(300):  # 最多50分钟
        ...
        if prompt_id in hist:
            ...
            for nid, out in outputs.items():
                for key in ['images', 'videos', 'videos_frames']:
                    if key in out:
                        for item in out[key]:
